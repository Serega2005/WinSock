...
extern int setup_for_ping(char* host, int ttl, SOCKET& sd, sockaddr_in& dest)
{
	...
	{
		//В противном случаем, предпологаем что host задан по имени, и пытаемся разрешить имя в адрес:
		hostent* hp = gethostbyname(host);
		if (hp != 0)
		{
			//Если имя разрешилось в адрес в IP-адрес, записываем этот адрес в dest
...
extern void init_ping_packet(ICMPHeader* icmp_hdr, int packet_size, int seq_no)
{
	...
	icmp_hdr->code = 0;
	icmp_hdr->checksum = 0;
	icmp_hdr->id = (USHORT)GetCurrentProcessId(); //Для тогоб чтобы отличать ICMP-сообщения, 
										   //если запущенно несколько экзампляров программы
	icmp_hdr->seq = seq_no;
...
{
	using namespace std;
	unsigned short header_lin = reply->h_len * 4;
	ICMPHeader* icmphdr = (ICMPHeader*)((char*)reply + header_lin);
	//При помощи арифметики указателей переходим с начала IP-загаловка

	if (bytes < header_lin + ICMP_MIN)
	{
	...
	else if (icmphdr->type != ICMP_ECHO_REPLY)
	{
		if (icmphdr->type != ICMP_TTL_EXPIRE)
		{
			...
			{
				cerr << "Unknow ICMP-packet type" << (int)icmphdr->type << endl;
			}
			return -1;
		}
	}
